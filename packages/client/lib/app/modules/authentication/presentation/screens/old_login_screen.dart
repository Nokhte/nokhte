...
/// Author: Sonny Vesali
///
/// This file defines the [LoginScreen] widget, which serves as a deprecated
/// basic login screen with two buttons for signing in with Apple and Google.
/// The login screen uses [StreamBuilder] to monitor the authentication state
/// and navigate to the home screen once the user is authenticated.
///
/// [LoginScreen] extends [StatefulWidget] and receives several stores and a
...
///
/// The [LoginScreen] widget listens to the [authStateStore.authState] stream.
/// If the stream data becomes true, indicating that the user is authenticated,
/// it calls the [addNameToDatabaseStore.call(NoParams())] method and navigates
/// to the home screen.
///
/// The UI of the [LoginScreen] contains two [PlatformElevatedButton] widgets,
/// one  for signing in with Apple and the other for signing in with Google.
/// Each button  triggers the [authProviderStore.routeAuthProviderRequest()]
/// method to initiate  the authentication process based on the selected
/// provider.
///
