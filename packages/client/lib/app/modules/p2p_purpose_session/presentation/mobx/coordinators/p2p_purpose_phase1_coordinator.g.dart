// GENERATED CODE - DO NOT MODIFY BY HAND

part of 'p2p_purpose_phase1_coordinator.dart';

// **************************************************************************
// StoreGenerator
// **************************************************************************

// ignore_for_file: non_constant_identifier_names, unnecessary_brace_in_string_interps, unnecessary_lambdas, prefer_expression_function_bodies, lines_longer_than_80_chars, avoid_as, avoid_annotating_with_dynamic, no_leading_underscores_for_local_identifiers

mixin _$P2PPurposePhase1Coordinator on _P2PPurposePhase1CoordinatorBase, Store {
  late final _$screenConstructorCallbackAsyncAction = AsyncAction(
      '_P2PPurposePhase1CoordinatorBase.screenConstructorCallback',
      context: context);

  @override
  Future screenConstructorCallback() {
    return _$screenConstructorCallbackAsyncAction
        .run(() => super.screenConstructorCallback());
  }

  late final _$joinTheCallAndMoveToPhase2AsyncAction = AsyncAction(
      '_P2PPurposePhase1CoordinatorBase.joinTheCallAndMoveToPhase2',
      context: context);

  @override
  Future joinTheCallAndMoveToPhase2() {
    return _$joinTheCallAndMoveToPhase2AsyncAction
        .run(() => super.joinTheCallAndMoveToPhase2());
  }

  @override
  String toString() {
    return '''

    ''';
  }
}
